...
int main(int argc, char** argv) {
    ...
    int nb;
    if(argc != 3){
        std::cerr << "Incorrect syntax: wikiall_gpu_ivf_flat <nb> <stats-path>\n";
        return -1;
    }
    ...
    faiss::Index* cpu_base = faiss::read_index("./gpu-ivf-flat.index");

    faiss::gpu::StandardGpuResources res;
    faiss::Index* gpu_base = faiss::gpu::index_cpu_to_gpu(&res, 0, cpu_base);
    delete cpu_base;
    auto* index = dynamic_cast<faiss::gpu::GpuIndexIVFFlat*>(gpu_base);
    if (!index) {
        std::cerr << "Failed to load GPU IVF-Flat index.\n";
        return -1;
    }
