...
int main(int argc, char** argv) {
    ...
    int nb;
    if(argc != 3){
        std::cerr << "Incorrect syntax: wikiall_cpu_ivf_pq_fastscan <nb> <stats-path>\n";
        return -1;
    }
    ...
    faiss::Index* base = faiss::read_index("./cpu-ivf-pq-fastscan.index");
    auto* index = dynamic_cast<faiss::IndexIVFPQFastScan*>(base);
    if (!index) {
    ...
    stats << "cpu-ivf-pq-fastscan,";   // index
    stats << nq << ",";         // nq
    stats << k << ",";          // k
